...
task("deploy-factory", "Deploys an GovernFactory instance").setAction(
  async (_, { ethers }) => {
    ...
    const GovernFactory = await GovernFactory.deploy(process.env.REGISTRY_RINKEBY, queueFactory.address)
...
task("deploy-Govern", "Deploys an Govern from provided factory")
  ...
  .setAction(async ({ factory: factoryAddr, useProxies, name }, { ethers }) => {
    ...
    const GovernFactory = await ethers.getContractAt('GovernFactory', factoryAddr)
