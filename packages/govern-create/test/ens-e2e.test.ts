...
class EnsHelper extends Ens {
  ...
  static async createEnsHelper(
    ...
    const controllerAddress = await resolver.interfaceImplementer(
      ethNodehash,
      InterfaceID_Controller
    )
    const registrarAddress = await ens.getOwner('eth')

    const ensHelper = new EnsHelper(
      provider,
      networkInfo,
      controllerAddress,
      registrarAddress
  ...
  async commit(args: any) {
    ...
    try {
      const commitment = await this.controller.makeCommitmentWithConfig(
        domain,
        owner,
        salt,
        resolver,
  ...
  async register(args: any) {
    ...
    try {
      ...
      const tx = await this.controller.registerWithConfig(
        domain,
        owner,
        duration,
        salt,
  ...
  async addSubdomain(args: any) {
    ...
    try {
      ...
      const subnodeTx = await this.ens.setSubnodeRecord(
        nodehash,
        labelhash,
        owner,
        resolver,
...
describe('ENS', function () {
  ...
  it('set address by non-owner should throw', async function () {
    ...
    const ens = new Ens(network.provider, ensNetwork)
    await expect(ens.setAddr(`${anotherDomain}.eth`, wallet.address)).to.be
      .reverted
  })
