...
import { 
  ...
  GovernFactoryMock__factory, GovernFactoryMock,
  GovernTokenFactoryMock__factory, GovernTokenFactoryMock,
  GovernBaseFactory, GovernBaseFactory__factory
} from '../typechain'
...
describe('Govern Base Factory with mocked contracts', function () {
  ...
  before(async () => {
    ...
    GovernBaseFactory = await ((await ethers.getContractFactory(
      'GovernBaseFactory'
    )) as GovernBaseFactory__factory).deploy(GovernRegistryMock.address, GovernFactoryMock.address, GovernQueueFactoryMock.address, GovernTokenFactoryMock.address);
  ...
  async function deployDAO(deployToken:boolean, useProxies: boolean, name: string) {
    ...
    await expect(tx)
      ...
      .withArgs(GovernFactoryMock.address, GovernQueueFactoryMock.address, deployToken ? GovernTokenFactoryMock.address : customAddress, name, "0x")
