...
task('ens:set-addr', 'Set the ENS address record')
  ...
  .setAction(async ({ name, address }, env) => {
    ...
    try {
      ...
      if (!tx) {
        console.log(
          `INFO: The ens name has already been set to the given address`
        )
        return
      }
      ...
      if (
        authorisedAddress !== from &&
        authorisedAddress != constants.AddressZero
      ) {
        console.log(
          'The account may not have the authorisation to set the address record'
        )
...
task('ens:reclaim', 'Reclaim the ownership of an ENS name')
  ...
  .addOptionalParam(
    'print',
    'Print the transaction data only',
    true,
    types.boolean
  )
...
task('ens:subdomain', 'Create a new ENS subdomain and set the resolver')
  ...
  .setAction(
    async ({ parent, label, controller, resolver, gas, print }, env) => {
      const { network, ethers } = env
      const ens = await Ens.createEns(network.provider, print)
      ...
      try {
        ...
        const tx = print
          ? await ens.ens.populateTransaction.setSubnodeRecord(
              nodehash,
              labelhash,
              controller,
              resolverOrDefault,
          ...
          : await ens.ens.setSubnodeRecord(
              nodehash,
              labelhash,
              controller,
              resolverOrDefault,
...
task('ens:setsubnode', 'Create a new ENS subdomain')
  ...
  .setAction(async ({ parent, label, controller, print }, env) => {
    ...
    try {
      ...
      const tx = print
        ? await ens.ens.populateTransaction.setSubnodeOwner(
            nodehash,
            labelhash,
            controller
          )
...
task('ens:setrecord', 'Create a new ENS subdomain')
  ...
  .addOptionalParam(
    'ttl',
    'The caching time-to-live of the ENS name',
    0,
    types.int
  )
  .addOptionalParam(
  ...
  .setAction(async ({ name, controller, resolver, print }, env) => {
    ...
    try {
      ...
      const tx = print
        ? await ens.ens.populateTransaction.setRecord(
            nodehash,
            controller,
            resolver,
            ttl
