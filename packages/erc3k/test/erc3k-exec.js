...
describe('ERC3000 Executor', function() {
  ...
  beforeEach(async () => {
    const ERC3000Executor = await ethers.getContractFactory('ERC3000ExecutorMock')
  ...
  context('ERC-165', () => {
    ...
    it('supports ERC-165', async () => {
      expect(await erc3kExec.supportsInterface(ERC165_INTERFACE_ID)).to.equal(true)
    })

    it('supports ERC-3000 Executor', async () => {
      expect(await erc3kExec.supportsInterface(await erc3kExec.interfaceID())).to.equal(true)
    })
