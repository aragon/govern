...
function decimalAmount(amount, decimals) {
    return BigNumber.from(amount).mul(BigNumber.from(10).pow(BigNumber.from(decimals)));
}
...
task('deploy-token', 'Uses factory to deploy a token and minter instances')
  ...
  .setAction(
    async(
        ...
        const tokenFactory = factoryAddr ? await ethers.getContractAt('GovernTokenFactory', factoryAddr)
                                        : await ethers.getContractAt('GovernTokenFactory', tokenFactoryDeployment.address)
        
        mintAmount = decimalAmount(mintAmount, decimals)

        const tx = await tokenFactory.newToken(initialMinter, name, symbol, decimals, minterAddress, mintAmount , useProxies, {
