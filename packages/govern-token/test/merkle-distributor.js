...
describe('MerkleDistributor', () => {
  ...
  describe('two account tree', () => {
    ...
    it('returns unclaimed balance', async () => {
      const proof = tree.getProof(0, wallet0, BigNumber.from(100))
      expect(
        await distributor.unclaimedBalance(0, wallet0, 100, proof)
      ).to.equal(100)
    })
    ...
    it('returns 0 for invalid proof', async () => {
      expect(
        await distributor.unclaimedBalance(0, wallet0, 100, [ZERO_BYTES32])
      ).to.equal(0)
    })
    ...
    it('cannot allow two claims', async () => {
      ...
      ).to.be.revertedWith('dist: already claimed')
    })
    ...
    it('cannot claim more than once: 0 and then 1', async () => {
      ...
      await expect(
        distributor.claim(0, wallet0, 100, proof1)
      ).to.be.revertedWith('dist: already claimed')
    })
    ...
    it('cannot claim more than once: 1 and then 0', async () => {
      ...
      await expect(
        distributor.claim(1, wallet1, 101, proof1)
      ).to.be.revertedWith('dist: already claimed')
    })
    ...
    it('cannot claim for address other than proof', async () => {
      ...
      ).to.be.revertedWith('dist: proof failed')
    })
