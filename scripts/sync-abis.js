...
async function main() {
  KNOWN_ABIS.map((abiName) => {
    ...
    console.log(
      fs.existsSync(`packages/govern/artifacts/${abiName}`),
      `found ${abiName}`
    );
    const rawAbiFile = fs.readFileSync(`packages/govern/artifacts/${abiName}`);
    ...
    fs.writeFileSync(
      `packages/govern-subgraph/abis/${abiName}`,
      stringifiedAbi
    );
  });
